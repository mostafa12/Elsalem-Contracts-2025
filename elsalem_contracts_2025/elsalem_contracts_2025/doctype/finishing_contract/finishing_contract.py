...
#
#
# class FinishingContract(SellingController):
# 	def __init__(self, *args, **kwargs):
# 		super(FinishingContract, self).__init__(*args, **kwargs)
#
# 	def validate(self):
# 		super(FinishingContract, self).validate()
...
# 			so = frappe.db.sql(
# 				"select name from `tabFinishing Contract` \
...
# 		super(FinishingContract, self).validate_warehouse()
#
# 		for d in self.get("items"):
# 			if (
# 				(
...
# 		super(FinishingContract, self).validate_with_previous_doc(
...
# 		super(FinishingContract, self).on_cancel()
#
...
# 		mod_db = frappe.db.get_value("Finishing Contract", self.name, "modified")
...
# 	for name in names:
# 		so = frappe.get_doc("Finishing Contract", name)
# 		if so.docstatus == 1:
# 			if status == "Closed":
